...
boxplot(log2p(inputcounts(res_ps))) # plotting zero-inflated counts
...
```{r}
boxplot(t(res_ps))
boxplot(t(deinflatedcounts(res_ps))) # equivalent
```
...
```{r}
plot(as.vector(deinflatedcounts(res_ps)),xlab="Index",ylab="counts")
```
...
```{r}
dds_zi <- zi2deseq2(res_ps, ~DiseaseState)
...
```{r}
deseq <- phyloseq_to_deseq2(ps, ~DiseaseState)
